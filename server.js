...
function updateComment(url, request) {
  ...
  if (savedComment && requestComment && requestComment.body &&
    database.comments[id].id === requestComment.id && 
    database.comments[id].username === requestComment.username) {
...
function deleteComment(url, request) {
  ...
  if (savedComment) {
    ...
    //delete an existing comment ID from author's comment IDs
    const userCommentIds = database.users[savedComment.username].commentIds;
    userCommentIds.splice(userCommentIds.indexOf(id), 1);
    //delete an existing comment ID from article's comment IDs
    const articalCommentIds = database.articles[savedComment.id].commentIds;
    articalCommentIds.splice(articalCommentIds.indexOf(id), 1);
    //successful delete
    response.status = 204;
  } else {
    //non-existant comment
    response.status = 404;
  }
